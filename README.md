...
1. Asegurate de tener un mapa guardado (por defecto `~/ros2_ws/maps/generated_map.yaml` generado con `map_saver_node`).
2. Lanzar la simulación igual que en la Parte A (Terminal 1).
3. Lanzar la navegación:

```
source install/setup.bash
ros2 launch tpf_navigation navigation_launch.py map_yaml:=~/ros2_ws/maps/generated_map.yaml enable_rviz:=true
```
